...
To a good approximation the Earth--Sun system is Keplerian. The Earth moves on an elliptical orbit with semimajor axis :math:`a = 1~\text{AU}` and ellipticity :math:`e = 0.0167` in the gravitational central potential generated by the Sun, which has mass :math:`m = 1~\text{M}_{\odot}`. We may represent this system in Dyad using the class :class:`dyad.Orbit`.
...
This is given in the format :math:`(x, y, z, v_{x}, v_{y}, v_{z})`
where :math:`x`, :math:`y`, and :math:`z` are in units of
:math:`\mathrm{AU}` and :math:`v_{x}`, :math:`v_{y}`, and
:math:`v_{z}` are in units of :math:`\mathrm{AU}~\mathrm{d}^{-1}`. We
can also compute the orbital radius and speed of the body, which are
also in units of :math:`\mathrm{AU}` and
:math:`\mathrm{AU}~\mathrm{d}^{-1}`.
...
They also include more interesting constants, such as the period (in :math:`\mathrm{d}`), periapsis (in :math:`\mathrm{AU}`) and apoapsis (in :math:`\mathrm{AU}`). A full list is available in the API documentation for :class:`dyad.Orbit`.
...
If the perifocal frame differs from the observer's frame we can specify its orientation using the Euler angles :math:`\Omega`, :math:`i`, and :math:`\omega` (longitude of the ascending node, inclination, and argument of pericentre). Suppose that :math:`\Omega = 1`, :math:`i = 1`, and :math:`\omega = 1`.
...
We can use the keyword ``theta`` to specify the true anomaly. In this case the function then returns :math:`\Theta_{\lambda}` also.
...
We may wish to know the orbital state of the two stars when :math:`\theta = 1`.
...
We might also like to know the system's other attributes. For example,
the periods and periapses of the two stars.
...
Of course we might equivalently compute these last two values as follows.
...
computed in the observer's frame.
