...
def _lazywhere(cond, arrays, f, fillvalue=None, f2=None):
    ...
    if f2 is None:
        # If `fillvalue` is a Python scalar and we convert to
        # `xp.asarray`, it gets the default `int` or `float` type of
        # `xp`, so `result_type` could be wrong.  `result_type`
        # should/will handle mixed array/Python scalars; remove this
        # special logic when it does.
