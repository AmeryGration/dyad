...
r"""Sample of the secondary-mass distribution of Moe and Stefano (2017)
...
#############################################################################
def f(p, q, m):
    """Return joint probability density for mass ratio and period"""
    res = (
        mass_ratio.moe2017(np.log10(p), m).pdf(q)
        *period.moe2017(m).pdf(p)
    )
    res = np.where((0.08 <= q*m) & (q*m <= 150.), res, 0.)
...
#     np.linspace(0.08, 0.8, n),
#     np.linspace(0.0, 1.2, n)[1:],
...
# Save data
#############################################################################
np.savetxt("./primary_mass_extended_sample.dat", primary_mass_sample)
np.savetxt("./mass_ratio_extended_sample.dat", mass_ratio_sample)
np.savetxt("./frequency_extended_sample.dat", pdf_sample)
np.savetxt("./cumulative_frequency_extended_sample.dat", cdf_sample)
