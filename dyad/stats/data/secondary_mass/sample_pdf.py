...
r"""Sample of the mass-ratio distribution implied by Moe and Stefano (2017)

We require the PDF for mass ratio (conditional on primary mass). This
is not given by Moe and Stefano (2017) but can be computed as follows.
...
Then, by the chain rule for probability,
...
def f(p, q, m):
    ...
    res = mass_ratio.moe2017(np.log10(p), m).pdf(q)*period.moe2017(m).pdf(p)#/m
...
print(trapezoid(pdf_sample, mass_ratio_sample))
