...
class uniform_gen(_distn_infrastructure.rv_continuous):
    r"""The secondary-star mass random variable for uniform pairing
    ...
    .. math::
       f_{M_{2}|M_{1} = m_{1}}(m_{2}|m_{1}) = \dfrac{1}{m_{\primary}(1 - \max(q_{\min}, m_{\min}/m_{\primary}))}

    with support on the region

    .. math::
       ((m_{\secondary}, m_{\primary}): (m_{\secondary}, m_{\primary}) \in I \times I \wedge m_{\secondary} \le m_{\primary} \wedge m_{\secondary} \ge q_{\min}m_{\primary}).
    ...
    def _pdf(self, x, primary_mass, mass_min, mass_ratio_min):
        # x = np.asarray(x)
        # primary_mass = np.asarray(primary_mass)
        # mass_ratio_min = np.asarray(mass_ratio_min)
        num = np.ones_like(x)
        denom = (
            primary_mass
            *(1. - np.maximum(mass_ratio_min, mass_min/primary_mass))
        )
        res = num/denom
        
        return res

    def _cdf(self, x, primary_mass, mass_min, mass_ratio_min):
        # x = np.asarray(x)
        # primary_mass = np.asarray(primary_mass)
        # mass_ratio_min = np.asarray(mass_ratio_min)
        res = x*_pdf(x, primary_mass, mass_min, mass_ratio_min)
...
uniform = uniform_gen(a=0.08, b=150., name="secondary_mass.uniform")
