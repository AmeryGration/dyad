...
def delaunay_elements_from_orbital_elements(a, e, Omega, i, omega, theta, m):
    ...
    where :math:`a` is the semimajor axis, :math:`e` is the
    eccentricity, :math:`\Omega` is the longitude of the ascending
    node, :math:`i` is the inclindation, :math:`\omega` is the
    argument of pericentre, and :math:`\mu(\theta)` is the mean anomaly.
    ...
    ...     0., 1.)
    (0.017202098944262438, 0.017202098944262438, 0.017202098944262438, 0.0, 0.0, 0.0)
    ...
    res = np.array([J_1, J_2, J_3, Theta_1, Theta_2, Theta_3]).T
...
def orbital_elements_from_delaunay_elements(
    ...
    .. math::
       ...
       a = \drac{J_{3}^{2}}{\mathrm{G}m}
       e = \sqrt(1 - \dfrac{J_{2}^{2}}{J_{3}^{2})
       \theta = \theta(\Theta_{3})
       \Omega = \Theta_1
       i = \cos^{-1}(J_{1}/J_{2})
       \omega = \Theta_{2}

    where :math:`J_{1}`, :math:`J_{2}`, :math:`J_{3}`,
    :math:`\Theta_{1}`, :math:`\Theta_{2}`, :math:`\Theta_{3}`, are
    ...
    >>> import dyad
    >>> dyad.orbital_elements_elements_from_delaunay_elemennts()
    ...
    if np.any(J_2 < 0.):
        raise ValueError("J_2 must be nonnegative.")
    if not np.all(np.isreal(J_3)):
        raise TypeError("J_3 must be scalar.")
    if np.any(J_3 < 0.):
        raise ValueError("J_3 must be nonnegative.")
    ...
    e = np.sqrt(1. - J_2**2./J_3**2.)
    theta = dyad.true_anomaly_from_mean_anomaly(Theta_3, e)
    Omega = Theta_1
    i = np.arccos(J_1/J_2)
    omega = Theta_2
    res = np.array([a, e, Omega, i, omega, theta]).T
...
def orbital_elements_from_modified_delaunay_elements(
    ...
    .. math::
       ...
       e = \sqrt{1 - (1 - \dfrac{J_{\varpi}}{J_{\lambda}}))^{2}}
       \theta = \theta(\Theta_{\lambda} + \Theta_{\varpi})
       \Omega = -\Theta_{\Omega}
       i = \cos^{-1}\left(1 - \dfrac{J_{\Omega}}{J_{\lambda} - J_{\varpi}}\right)
       \omega = -\Theta_{\varpi} + \Theta_{\Omega}

    where :math:`J_{\varpi}`, :math:`J_{\Omega}`, :math:`J_{\lambda}`,
    :math:`\Theta_{\varpi}`, :math:`\Theta_{\Omega}`,and
    :math:`\Theta_{\lambda}`, are the modified Delaunay elements.
    ...
    >>> import dyad
    >>> dyad.orbital_elements_elements_from_modified_delaunay_elemennts()
    ...
    if np.any(J_lambda < 0.):
        raise ValueError("J_lambda must be nonnegative.")
