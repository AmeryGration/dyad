...
class Orbit:
    ...
    @property
    def period(self):
        return 2.*self.area/self.angular_momentum_magnitude
...
class Binary:
    def __init__(self, m, q, a, e, theta, Omega, i, omega):
        ...
        self.primary = Orbit(m, elements1)
        self.secondary = Orbit(m*q, elements2)
...
e1 = 0.
P = np.sqrt(
    4.*np.pi**2.*(a1 + a2)**3./(GRAV_CONST*(m1 + m2))
)

bin = Binary(m1, m2/m1, a1, e1, 0., 0., 0., 0.)

print(P)
print(bin.primary.period)
print(bin.secondary.period)
print(bin.primary.period/P)
