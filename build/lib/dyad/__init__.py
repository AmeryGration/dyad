...
.. figure:: ../figures/orbital_euler_angles.jpg
   ...
   the coordinate system of the observer described by the Euler angles
   :math:`\Omega`, :math:`i`, and :math:`\omega`. The unit vector
   :math:`\mathbf{e}_{N}` is parallel with the *line of nodes*
...
xxx. Both bodies move on elliptical orbits with primary foci
coincident with the system's centre of mass (COM) frame. The
more-massive (resp. less-massive) body moves as if in a central
potential generated by a mass of :math:`m_{2}^{3}/(m_{1} + m_{2})^{2}`
(resp. :math:`m_{1}^{3}/(m_{1} + m_{2})^{2}`). Its orbit is described
by its semimajor axis, :math:`a_{1}` (resp. :math:`a_{2}`), and
eccentricity, :math:`e_{1}` (resp. :math:`e_{2}`). Its orientation is
defined by the longitude of its ascending node, :math:`\Omega_{1}`
(resp. :math:`\Omega_{2}`), inclination, :math:`i_{1}`
(resp. :math:`i_{2}`), and argument of pericentre, :math:`\omega_{1}`
(resp. :math:`\omega_{2}`). The phase of the more-massive
(resp. less-massive) body is described by its true anomaly,
:math:`\theta_{1}` (resp. :math:`\theta_{2})`. It is the case that
