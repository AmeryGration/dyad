...
import numpy as np
from scipy.stats._constants import _XMAX, _LOGXMAX
...
As an instance of the `rv_continuous` class, `dyad.stats.%(name)s` object inherits from it
...
rv = dyad.stats.%(name)s(%(shapes)s, loc=0, scale=1)
...
%(set_vals_stmt)s
>>> mean, var, skew, kurt = dyad.stats.%(name)s.stats(%(shapes)s, moments='mvsk')

Display the probability density function (``pdf``):

>>> x = np.linspace(dyad.stats.%(name)s.ppf(0.01, %(shapes)s),
...                 dyad.stats.%(name)s.ppf(0.99, %(shapes)s), 100)
>>> ax.plot(x, dyad.stats.%(name)s.pdf(x, %(shapes)s),
...        'r-', lw=5, alpha=0.6, label='dyad.stats.%(name)s pdf')
...
>>> rv = dyad.stats.%(name)s(%(shapes)s)
>>> ax.plot(x, rv.pdf(x), 'k-', lw=2, label='frozen pdf')

Check accuracy of ``cdf`` and ``ppf``:

>>> vals = dyad.stats.%(name)s.ppf([0.001, 0.5, 0.999], %(shapes)s)
>>> np.allclose([0.001, 0.5, 0.999], dyad.stats.%(name)s.cdf(vals, %(shapes)s))
True

Generate random numbers:

>>> r = dyad.stats.%(name)s.rvs(%(shapes)s, size=1000)
...
equivalent to ``dyad.stats.%(name)s.pdf(y, %(shapes)s) / scale`` with
...
rv = dyad.stats.%(name)s(%(shapes)s, loc=0)
...
>>> x = np.arange(dyad.stats.%(name)s.ppf(0.01, %(shapes)s),
...               dyad.stats.%(name)s.ppf(0.99, %(shapes)s))
>>> ax.plot(x, dyad.stats.%(name)s.pmf(x, %(shapes)s), 'bo', ms=8, label='dyad.stats.%(name)s pmf')
>>> ax.vlines(x, 0, dyad.stats.%(name)s.pmf(x, %(shapes)s), colors='b', lw=5, alpha=0.5)
...
>>> prob = dyad.stats.%(name)s.cdf(x, %(shapes)s)
>>> np.allclose(x, dyad.stats.%(name)s.ppf(prob, %(shapes)s))
True
...
Specifically, ``dyad.stats.%(name)s.pmf(k, %(shapes)s, loc)`` is identically
equivalent to ``dyad.stats.%(name)s.pmf(k - loc, %(shapes)s)``.
"""
...
def _sum_finite(x):
    ...
    --------
    >>> import numpy as np
    >>> from _distn_infrastructure import _sum_finite
...
def _iter_chunked(x0, x1, chunksize=4, inc=1):
    ...
    >>> from _distn_infrastructure import _iter_chunked
